...
// Assume that beast and dish are always lowercase strings, and that each has at least two letters. beast and dish may contain hyphens and spaces, but these will not appear 
//at the beginning or end of the string. They will not contain numerals.

function feast(beast, dish) {
    for(let i = 0; i < beast.length; i++){
      for(let e = 0; e < dish.length; e++){
         if(beast[i] === dish[i] && beast [beast.length -1] === dish [dish.length - 1]){
          return true;
    }else {
      return false;  
         }
